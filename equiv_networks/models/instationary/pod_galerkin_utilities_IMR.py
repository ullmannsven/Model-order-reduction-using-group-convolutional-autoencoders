...
def Galerkin_residuum(reduced_basis, x, xn_1, mu, dt, fom, u_ref, symplectic):
...
def Jacobian_approximate_Galerkin_residuum(reduced_basis, x, xn_1, mu, dt, fom, u_ref, symplectic):
...
def Galerkin_line_search(reduced_basis, x, p, xn_1, mu, dt, fom, u_ref, symplectic, min_stepsize=0.01, frac=0.9, c_1=1e-4, c_2=0.9):
    """Strong Wolfe line search."""
    alpha = 1.0

    res_orig = Galerkin_residuum(reduced_basis, x, xn_1, mu, dt, fom, u_ref, symplectic)
    ...
    res_update = Galerkin_residuum(reduced_basis, x + alpha * p, xn_1, mu, dt, fom, u_ref, symplectic)
...
def POD_Galerkin_quasi_newton(reduced_basis, xn_1, mu, dt, fom, u_ref, symplectic, tol=1e-8):
    """Quasi-Newton with midpoint residual."""
    x_new = xn_1
    res = Galerkin_residuum(reduced_basis, x_new, xn_1, mu, dt, fom, u_ref, symplectic)
    res_norm = np.linalg.norm(res)

    while res_norm > tol:
        J_approx = Jacobian_approximate_Galerkin_residuum(reduced_basis, x_new, xn_1, mu, dt, fom, u_ref, symplectic)
        p = np.linalg.solve(J_approx, -res.T).T

        alpha, res, res_norm = Galerkin_line_search(reduced_basis, x_new, p, xn_1, mu, dt, fom, u_ref, symplectic)
        x_new = x_new + alpha * p
