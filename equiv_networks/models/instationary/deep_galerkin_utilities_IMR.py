...
def Galerkin_residuum(model, x, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic):
...
def Jacobian_approximate_Galerkin_residuum(model, x, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic):
...
def Galerkin_line_search(model, x, p, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic, min_stepsize=5e-2, frac=0.9, c_1=1e-4, c_2=0.9):
    """Strong Wolfe line search."""
    alpha = 1.0

    res_orig = Galerkin_residuum(model, x, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic)
    ...
    res_update = Galerkin_residuum(model, x + alpha * p, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic)
...
def compute_exact_jacobian_fd(model, x, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic, eps=1e-7):
    """Compute exact Jacobian of residual using finite differences.
    
    The residual r(x) has shape (1, p_red)
    The Jacobian dr/dx has shape (p_red, p_red)
    """
    p_red = x.shape[1]
    r_x = Galerkin_residuum(model, x, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic)  # Shape: (1, p_red)
    
    J_exact = np.zeros((p_red, p_red))
    
    for i in range(p_red):
        x_pert = x.copy()
        x_pert[0, i] += eps
        r_pert = Galerkin_residuum(model, x_pert, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic)  # Shape: (1, p_red)
        J_exact[:, i] = (r_pert - r_x).flatten() / eps  # i-th column of Jacobian
    
    return J_exact

def Galerkin_quasi_newton(model, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic, tol=1e-8):
    """Quasi-Newton with midpoint residual."""
    model.network.decoder.eval()
    x_new = xn_1
    res = Galerkin_residuum(model, x_new, xn_1, mu, dt, fom, u_ref, scaled_data, symplectic)
