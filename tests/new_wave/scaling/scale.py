...
class Scaler:
    ...
    def __init__(self, dims, filename="./scaling_grid", eps=1e-12, device="cpu"):
        ...
        with open(f'{filename}_{self.Nx}x{self.Ny}_new_wave', "rb") as f:
    ...
    def scale(self, x):
        ...
        x = torch.as_tensor(x, dtype=torch.double, device=self.device)
        scaled = torch.empty_like(x, dtype=torch.double, device=self.device)
        if x.ndim == 3:
            q, p = x[0, :, :], x[1, :, :]
    ...
    def unscale(self, x):
        ...
        x = torch.as_tensor(x, dtype=torch.double, device=self.device)
        unscaled = torch.empty_like(x, dtype=torch.double, device=self.device)
        if x.ndim == 3:
            q, p = x[0, :, :], x[1, :, :]
