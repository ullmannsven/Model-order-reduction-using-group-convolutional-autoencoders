...
def test_wave_2D(): 
    ...
    experiment = WaveExperiment(config, mu_val=100) #dummy to get the filepaths
    p_red = 24
    ...
    filename = filepaths['snapshots'] / f"snapshots_{config.Nx}x{config.Ny}_080_nt_{config.nt}"
    with open(filename, 'rb') as file:
        arr = pickle.load(file)['snapshots']
    u_test = np.vstack(arr).T


    rb_path = rb_dir / f"reduced_basis_{config.Nx}x{config.Ny}_rbsize_24_nt_{config.nt}.npy"
    ...
    for i in range(config.n_timesteps):
        ...
        ## Quasi Newton:
        u_new = POD_Galerkin_quasi_newton(reduced_basis, u_n1, mu_test, config.dt, experiment.fom, u_ref, symplectic=False, tol=1e-8)
        ## End Quasi Newton
    ...
    with result_file.open('wb') as f:
        pickle.dump({'mu': mu_test, 'u_pod_galkerin': u_approx_full, 'u_full': u_test}, f)
