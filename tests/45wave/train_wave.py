#!/usr/bin/env python
"""
Train an Autoencoder on the 2D wave equation experiment (rotated/instationary variant).

Usage:
    python train_wave_2D_rotated.py --ae_name AE_NAME [--p_red P] [--epochs N] [--lr LR] [--batch_size B]
...
Examples:
    # C8 equivariant network, default settings
    python train_wave_2D_rotated.py --ae_name RotationUpsamplingGCNN_C8

    # CNN baseline, p_red=8
    python train_wave_2D_rotated.py --ae_name UpsamplingCNN --p_red 8

    # C4 network, custom learning rate and epochs
    python train_wave_2D_rotated.py --ae_name RotationUpsamplingGCNN_C4 --p_red 16 --lr 0.001 --epochs 500
"""
...
from equiv_networks import RotationUpsamplingGCNNAutoencoder2D
...
def train_wave_2D(ae_name, p_red=12, number_of_epochs=1000, learning_rate=0.0005, batch_size=20):
    ...
    for mu in [0.5, 0.75, 1.0]:
        filename = base_dir / f'snapshots_{Nx}x{Ny}_mu{mu}'
    ...
    scaling_dir = script_dir / "scaling"
    os.makedirs(scaling_dir, exist_ok=True)
    scaling_filename = scaling_dir / f"scaling_grid_{Nx}x{Ny}"
    ...
    for d in [checkpoints_dir, network_parameters_dir]:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train an Autoencoder on the 2D wave equation experiment (rotated/instationary variant).',
    ...
    train_wave_2D(ae_name=args.ae_name, p_red=args.p_red, number_of_epochs=args.epochs, learning_rate=args.lr, batch_size=args.batch_size)